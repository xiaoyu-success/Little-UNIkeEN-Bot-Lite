...
class UserBan(StandardPlugin):
    ...
    @staticmethod
    def checkBanAuthentication(
        banTarget: int, enforcementPersonnel: int, groupId: int
    ) -> Tuple[bool, str]:
        groupAdmins = getGroupAdmins(groupId)
        if enforcementPersonnel not in groupAdmins:
            return False, "非群BOT管理员无法使用该功能"
        if banTarget in ROOT_ADMIN_ID or isGroupOwner(
            group_id=groupId, user_id=banTarget
        ):
            return False, "群主和ROOT无法被ban"
        if enforcementPersonnel in ROOT_ADMIN_ID or isGroupOwner(
            group_id=groupId, user_id=enforcementPersonnel
        ):
            return True, ""
    ...
    def executeEvent(self, msg: str, data: Any) -> Optional[str]:
        ...
        if banTarget.isdigit():
            ...
            return
        if banType == "ban":
            authSucc, authReason = self.checkBanAuthentication(
                banTarget, enforcementPersonnel, groupId
            )
            if authSucc:
            ...
            authSucc, authReason = self.checkUnbanAuthentication(
                banTarget, enforcementPersonnel, groupId
            )
            if authSucc:
...
def drawGroupBanInfo(
    groupId: int, groupBanInfo: List[Tuple[int, int, datetime.datetime]], savePath: str
) -> Tuple[bool, str]:
    card = ResponseImage(title="机器人ban列表", footer="群 %d" % groupId)
    if len(groupBanInfo) == 0:
        card.addCard(ResponseImage.NoticeCard(title="空空如也~"))
    else:
        content = []
        for banTarget, enforcementPersonnel, banTime in groupBanInfo:
            content.append(
                (
                    "title",
                    "被ban用户 %d, 管理员 %d, 被ban时间 %s"
                    % (
                        banTarget,
                        enforcementPersonnel,
                        banTime.strftime("%Y-%m-%d %H:%M:%S"),
                    ),
                )
            )
...
class GetBanList(StandardPlugin):
    def judgeTrigger(self, msg: str, data: Any) -> bool:
        return msg in ["-banlist"] and data["user_id"] in getGroupAdmins(
            groupId=data["group_id"]
        )
