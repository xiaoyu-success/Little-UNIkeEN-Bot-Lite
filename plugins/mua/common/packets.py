...
class PayloadPacket(Packet):
    ...
    def as_subprotocol_packet(self):
        if (
            self.subprotocol_name in self.__SUBPROTOCOL_REGISTRY
            and self.subprotocol_packet_type
            in self.__SUBPROTOCOL_REGISTRY[self.subprotocol_name]
        ):
            return self.__SUBPROTOCOL_REGISTRY[self.subprotocol_name][
                self.subprotocol_packet_type
            ].from_payload_packet(self)
        return None
