...
class MuaTokenBinder(StandardPlugin):
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if len(tokenDescription) > 20:
            send(
                target,
                "[CQ:reply,id=%d]绑定失败，MUAID须在20字以内" % data["message_id"],
                data["message_type"],
            )
        elif len(token) > 100 or self.tokenPattern.match(token) == None:
            send(
                target,
                "[CQ:reply,id=%d]绑定失败，请检查token格式" % data["message_id"],
                data["message_type"],
            )
        else:
            try:
                ...
                if not succ:
                    send(
                        target,
                        "[CQ:reply,id=%d]token验证失败，请检查token" % data["message_id"],
                        data["message_type"],
                    )
                    return "OK"
                if verifiedMuaId != tokenDescription:
                    send(
                        target,
                        "[CQ:reply,id=%d]token绑定失败，用户输入的MUA ID和服务器返回MUA ID不一致"
                        % data["message_id"],
                        data["message_type"],
                    )
                    return "OK"
                ...
                send(
                    target,
                    "[CQ:reply,id=%d]与MUA服务器连接失败，无法验证token真实性，请尝试稍后绑定"
                    % data["message_id"],
                    data["message_type"],
                )
                return "OK"
            try:
                ...
                (%s, %s, %s)""",
                    (userId, tokenDescription, token),
                )
                send(
                    target,
                    "[CQ:reply,id=%d]绑定成功" % data["message_id"],
                    data["message_type"],
                )
                ...
                send(
                    target,
                    "[CQ:reply,id=%d]绑定失败，数据库错误" % data["message_id"],
                    data["message_type"],
                )
        return "OK"
...
def getUserMuaIdPermission(userId: int, muaId: str) -> int:
    mydb, mycursor = newSqlSession()
    mycursor.execute(
        """select `mua_token`, `empowered` from `muaToken` where 
    `user_id` = %s and `token_description` = %s""",
        (userId, muaId),
    )
...
def makeEmpower(userId: int, empowerTargetId: int, muaId: str) -> Tuple[bool, str]:
    ...
    if len(result) == 0:
        return (
            False,
            "您不存在名为“%s”的MUA ID，请输入“-muabind [MUAID] [TOKEN]”绑定之。经查询，您名下的MUA ID有：%s"
            % (muaId, "，".join(getAllMuaToken(userId).keys())),
        )
    token, empowered = result[0]
    if empowered:
        return False, "您名为“%s”的MUA ID是他人授予的，系统不支持MUA ID的递归授予，请联系MUA ID的持有者授权" % muaId
    mycursor.execute(
        """select count(*) from `muaToken` where 
    `user_id` = %s and `token_description` = %s""",
        (empowerTargetId, muaId),
    )
    if list(mycursor)[0][0] > 0:
        return False, "您的授予对象已有名为“%s”的MUA ID了，无法绑定重名MUA ID" % muaId
    mycursor.execute(
        """insert into `muaToken` (`user_id`, `token_description`, `mua_token`, `empowered`)
    values (%s, %s, %s, true)""",
        (empowerTargetId, muaId, token),
    )
    return True, "授权成功"
...
class MuaTokenEmpower(StandardPlugin):
    def __init__(self):
        ...
        self.triggerPattern2 = re.compile(
            r"^\-muaempower\s+(\S+)\s*\[CQ\:at\,qq=(\d+)\]"
        )
...
class MuaTokenLister(StandardPlugin):
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if len(tokens) == 0:
            send(
                target,
                "[CQ:reply,id=%d]您还没有绑定mua token" % (data["message_id"],),
                data["message_type"],
            )
        else:
            ...
            send(
                target,
                "[CQ:reply,id=%d]您的token有：%s" % (data["message_id"], result),
                data["message_type"],
            )
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        try:
            ...
            if list(mycursor)[0][0] == 0:
                send(
                    target,
                    "[CQ:reply,id=%d]您尚未绑定MUAID为 %s 的token，无法解绑"
                    % (data["message_id"], tokenDescription),
                    data["message_type"],
                )
            else:
                mycursor.execute(
                ...
                send(
                    target,
                    "[CQ:reply,id=%d]解绑成功" % data["message_id"],
                    data["message_type"],
                )
        except BaseException as e:
            print(e)
            send(
                target,
                "[CQ:reply,id=%d]解绑失败，数据库错误" % data["message_id"],
                data["message_type"],
            )
        return "OK"
