...
class AnnouncementFilter:
    ...
    def parseAsFilterJson(filterStr: str) -> Tuple[bool, Union[None, str]]:
        if filterStr.startswith("channel+:"):
            ...
            return True, json.dumps({"channel": {"whitelist": whitelist}})
        elif filterStr.startswith("channel-:"):
            blacklist = filterStr[9:].strip().split()
            return True, json.dumps({"channel": {"blacklist": blacklist}})
        else:
            return False, None
...
def rmGroupFilter(groupId: int) -> bool:
    mydb, mycursor = newSqlSession()
    mycursor.execute(
        """select count(*) from `muaGroupAnnFilter`
    where `group_id` = %s
    """,
        (groupId,),
    )
...
class MuaGroupAnnFilter(StandardPlugin):
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        admins = set(
            u["user_id"]
            for u in get_group_member_list(groupId)
            if u["role"] in ["admin", "owner"]
        ).union(getGroupAdmins(groupId))
        if userId not in admins:
            send(
                groupId,
                "[CQ:reply,id=%d]权限检查失败。该指令仅允许群管理员触发。" % data["message_id"],
                data["message_type"],
            )
            return "OK"
        if msg == "-rmannfilter":
            if not rmGroupFilter(groupId):
                send(
                    groupId,
                    "[CQ:reply,id=%d]没有设置过订阅频道。" % data["message_id"],
                    data["message_type"],
                )
            else:
                send(
                    groupId,
                    "[CQ:reply,id=%d]删除成功。" % data["message_id"],
                    data["message_type"],
                )
            return "OK"
