...
def groupUnbindTarget(groupId: int, muaTarget: str) -> Tuple[bool, str]:
    mydb, mycursor = newSqlSession()
    mycursor.execute(
        """select count(*) from `muaGroupTarget`
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if msg == "-muagroupbind":
            ...
            if len(muaTargets) == 0:
                send(
                    groupId,
                    "[CQ:reply,id=%d]当前群尚未绑定MUA ID，请输入“-muagroupbind [绑定的MUA ID]”进行target绑定"
                    % data["message_id"],
                )
            else:
                send(
        ...
        if len(result) == 0:
            send(
                groupId,
                (
                    "[CQ:reply,id=%d]指令识别失败。输入“-muagroupbind”可以获取群已绑定的MUA ID，输入“-muagroupbind [绑定的MUA ID]”"
                    "可以继续绑定，输入“-muagroupunbind [绑定的MUA ID]可以解除绑定”"
                )
                % (data["message_id"],),
            )
