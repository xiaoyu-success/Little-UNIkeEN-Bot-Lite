...
class MuaAnnHelper(StandardPlugin):
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if drawHelpPic(savePath):
            ...
            send(
                target,
                "[CQ:reply,id=%d]帮助生成失败" % data["message_id"],
                data["message_type"],
            )
        return "OK"
...
class MuaAnnEditor(StandardPlugin):
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if msg == "-annkey":
            ...
            if annKey == None:
                send(
                    target,
                    "[CQ:reply,id=%d]当前没有正在编辑的通知" % data["message_id"],
                    data["message_type"],
                )
            else:
                send(
                    target,
                    "[CQ:reply,id=%d]当前通知的关键字为“%s”, %s"
                    % (
                        data["message_id"],
                        annKey[0],
                        "未发布可编辑" if annKey[1] else "已发布不可编辑",
                    ),
                    data["message_type"],
                )
        elif msg == "-annls":
            ...
            send(
                target,
                "[CQ:reply,id=%d]%s" % (data["message_id"], result),
                data["message_type"],
            )
        elif msg == "-annrmctt":
            ...
            if succ:
                send(
                    target,
                    "[CQ:reply,id=%d]OK" % (data["message_id"],),
                    data["message_type"],
                )
                with open(
                    os.path.join(ROOT_PATH, SAVE_TMP_PATH, "anndmp.json"), "w"
                ) as f:
                    f.write(result)
            else:
                send(
            ...
            send(
                target,
                "[CQ:reply,id=%d]指令识别失败，请发送-annhelp获取帮助" % data["message_id"],
                data["message_type"],
            )
    ...
    def annDmp(self, userId: int, annKey: Optional[str], data: Any) -> Tuple[bool, str]:
        if annKey == None:
            ...
            if curKey == None:
                return False, "没有给关键词且当前没有正在编辑的通知"
            annKey = curKey[0]
    ...
    def annNew(self, userId: int, annKey: str, data: Any) -> Tuple[bool, str]:
        ...
        try:
            ...
            where `user_id` = %d and `editing` = true"""
                % (userId,)
            )
            try:
                mycursor.execute(
                    """insert into `muaAnnCtx` 
            ...
            return True, (
                "创建成功，发送“-anntk MUAID”可为通知绑定发布身份的MUAID(若不清楚MUAID名称，发送“-muals”可获取已注册的MUAID)，"
    ...
    def annSwt(self, userId: int, annKey: str, data: Any) -> Tuple[bool, str]:
        ...
        mycursor.execute(
            """update `muaAnnCtx` set
        `editing` = false where `user_id` = %s and `editing` = true""",
            (userId,),
        )
        mycursor.execute(
            """update `muaAnnCtx` set
    ...
    def annCtt(self, userId: int, content: str, data: Any) -> Tuple[bool, str]:
        ...
        if not annKey[1]:
            return False, (
                "content删除失败，当前通知已发布，无法编辑，" "如需修改，请用-anncp命令将该通知复制到新通知，然后编辑发布"
            )
        annKey: str = annKey[0]
        ...
        try:
            ...
            return True, (
                "content添加成功，可以继续发送“-annctt 内容”往现有内容后面添加新图文（不会覆盖），"
    ...
    def annCp(
        self, userId: int, srcKey: str, dstKey: str, data: Any
    ) -> Tuple[bool, str]:
        mydb, mycursor = newSqlSession()
        # 1. 看dstKey是不是被占用了
        mycursor.execute(
        ...
        if list(mycursor)[0][0] > 0:
            return (
                False,
                "关键词“%s”被占用，请用-annrm删掉之前的通知(-annrm只会删除本地记录，不会请求服务器删除)，或换一个没有被占用的关键词"
                % dstKey,
            )
        # 2. 读取
        mycursor.execute(
            """select `title`, `content`, `begin_time`, `end_time`, `info_source`,
    ...
    def annTtl(self, userId: int, title: str, data: Any) -> Tuple[bool, str]:
        ...
        if not annKey[1]:
            return False, ("title设置失败，当前通知已发布，无法编辑，" "如需修改，请用-anncp命令将该通知复制到新通知，然后编辑发布")
        annKey: str = annKey[0]
        if len(title) > 100:
        ...
        try:
            ...
            return (
                True,
                "title设置成功，发送“-anntg 标签(不同标签以空格隔开)”可以编辑通知标签，发送“-annctt 内容”可以编辑通知内容",
            )
        except BaseException as e:
    ...
    def annCnl(self, userId: int, channel: str, data: Any) -> Tuple[bool, str]:
        ...
        if not annKey[1]:
            return False, (
                "channel设置失败，当前通知已发布，无法编辑，" "如需修改，请用-anncp命令将该通知复制到新通知，然后编辑发布"
            )
        annKey: str = annKey[0]
        if len(channel) > 200:
        ...
        try:
            ...
            return True, (
                "channel设置成功，请发送“-annttl 标题”编辑通知标题，"
    ...
    def annTgt(self, userId: int, tgt: str, data: Any) -> Tuple[bool, str]:
        ...
        if not annKey[1]:
            return False, (
                "target设置失败，当前通知已发布，无法编辑，" "如需修改，请用-anncp命令将该通知复制到新通知，然后编辑发布"
            )
        annKey: str = annKey[0]
    ...
    def annTg(self, userId: int, tag: str, data: Any) -> Tuple[bool, str]:
        ...
        if not annKey[1]:
            return False, ("tag设置失败，当前通知已发布，无法编辑，" "如需修改，请用-anncp命令将该通知复制到新通知，然后编辑发布")
        annKey: str = annKey[0]
        ...
        try:
            ...
            return True, (
                "tag设置成功，继续发送“-annctt 内容”可以编辑通知内容，"
    ...
    def annStt(self, userId: int, timeStr: str, data: Any) -> Tuple[bool, str]:
        ...
        if not annKey[1]:
            return False, ("开始时间设置失败，当前通知已发布，无法编辑，" "如需修改，请用-anncp命令将该通知复制到新通知，然后编辑发布")
        annKey: str = annKey[0]
        invalidTime = False
        if timeParsed == None:
            invalidTime = True
    ...
    def annStp(self, userId: int, timeStr: str, data: Any) -> Tuple[bool, str]:
        ...
        if not annKey[1]:
            return False, ("结束时间设置失败，当前通知已发布，无法编辑，" "如需修改，请用-anncp命令将该通知复制到新通知，然后编辑发布")
        annKey: str = annKey[0]
        timeParsed = parseTimeStr(timeStr)
        invalidTime = False
        if timeParsed == None:
    ...
    def annPrv(self, userId: int, data: Any) -> Tuple[bool, str]:
        ...
        if result["end_time"] != None:
            txt += datetime.datetime.fromtimestamp(result["end_time"]).strftime(
                "结束时间： %Y-%m-%d %H:%M:%S\n\n"
            )
        else:
            txt += "结束时间缺失\n\n"
    ...
    def annTk(self, userId: int, tokenDescription: str, data: Any):
        ...
        if annKey == None:
            return False, (
                "MUA ID设置失败，当前没有正在编辑的通知，"
                "请发送“-annnew 新通知关键字”创建新通知，或者发送“-anncp 已创建关键字 新通知关键字”从模板中复制新通知"
            )
        if not annKey[1]:
            return False, (
                "MUA ID设置失败，当前通知已发布，无法编辑，" "如需修改，请用-anncp命令将该通知复制到新通知，然后编辑发布"
            )
        annKey: str = annKey[0]
        ...
        if tokenDescription not in allToken:
            return (
                False,
                f'绑定失败，您注册的所有MUA ID为：{"、".join(allToken)}，在其中没有发现名为“{tokenDescription}”的ID',
            )
        try:
            mydb, mycursor = newSqlSession()
            mycursor.execute(
            ...
            return True, (
                "MUA ID设置成功，请发送“-anncnl 目标频道”设置频道（原版/模组/小游戏/其他），"
    ...
    def annRls(self, userId: int, data: Any) -> Tuple[bool, str]:
        ...
        if not annKey[1]:
            return False, ("发布失败，当前通知已发布，无法重新发布，" "如需重新发布，请用-anncp命令将该通知复制到新通知，然后编辑发布")
        annKey: str = annKey[0]
        # do some check
        result = loadAnnContext(userId, annKey)
        if result == None:
