...
def drawMuaListPic(
    savePath: str,
    muaList: Dict[str, List[Dict[str, Any]]],
    groupAnnFilter: AnnouncementFilter,
) -> Tuple[bool, str]:
    """绘制MUA通知
    @savePath: 图片保存路径
    @muaList:  mua通知
    ...
    try:
        ...
        for channel, announcements in muaList.items():
            ...
            for announcement in announcements:
                ...
                if is_new:
                    raw_content.append(
                        ("body", "[新] " + announcement["title"], PALETTE_RED)
                    )
                else:
...
def handle_payload_fn(session_id, payload):
    ...
    if data != None:
        ...
        if retType == "RESULT":
            ...
            send(
                data["target"],
                f'[CQ:reply,id={data["message_id"]}]succ: {succ}, reason: {reason}',
                data["message_type"],
            )
            if succ:
                aid = body["aid"]
            ...
            savePath = os.path.join(
                ROOT_PATH, SAVE_TMP_PATH, "mualist_%d.png" % data["user_id"]
            )
            if data["abstract"]:
                succ, result, _ = drawMuaBriefListPic(
                    savePath, body, getGroupFilter(data["target"])
                )
            else:
                succ, result = drawMuaListPic(
                    savePath, body, getGroupFilter(data["target"])
                )
            if succ:
                send(
        ...
        if retType == "LIST":
            ...
            for groupId in getPluginEnabledGroups("muanotice"):
                savePath = os.path.join(
                    ROOT_PATH, SAVE_TMP_PATH, "mualist_broadcast_%d.png" % groupId
                )
                succ, result, shouldSend = drawMuaBriefListPic(
                    savePath, body, getGroupFilter(groupId)
                )
                if succ:
                    if shouldSend:
