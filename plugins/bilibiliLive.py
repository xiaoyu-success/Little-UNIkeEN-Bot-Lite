...
def createBilibiliLiveSql():
    ...
    mycursor.execute(
        """create table if not exists `biliLiveStatus`(
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if roomInfo["live_status"] == 1:
            savePath = os.path.join(
                ROOT_PATH,
                SAVE_TMP_PATH,
                "biliLive-%s-%d.png" % (self.liveDescription, target),
            )
...
class BilibiliLiveMonitor(StandardPlugin, CronStandardPlugin):
    ...
    def loadLiveStatus(
        ...
        mydb, mycursor = newSqlSession(autocommit=True)
        mycursor.execute(
            """select `liveStatus` from `biliLiveStatus`
        where `liveId` = %s""",
            (self.liveId,),
        )
    ...
    def tick(self):
        ...
        if currentStatus != self.prevStatus:
            ...
            if currentStatus:
                savePath = os.path.join(
                    ROOT_PATH, SAVE_TMP_PATH, "biliLive-%d.png" % self.liveId
                )
                genLivePic(
                    roomInfo, "%s直播间状态" % self.description, savePath, useCover=True
                )
                for group in getPluginEnabledGroups(self.targetGroup):
                    send(
                        group,
                        "检测到%s开播，B站直播地址： https://live.bilibili.com/%d"
                        % (self.description, self.liveId),
                    )
