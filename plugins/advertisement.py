...
class BaseAdvertisementClass(CronStandardPlugin):
    ...
    def __init__(
        self, adword: str, group_id: int, uniqueKey: str, deltaTime: int, deltaMsg: int
    ):
        """
        @group_id: 投放广告的目标群组
    ...
    def dumpContext(self, lastMsgSeq: int, lastTime: int):
        mydb, mycursor = newSqlSession()
        mycursor.execute(
            """replace into `advertisementContext`
        (`last_message_seq`,`last_time`,`group_id`, `unique_key`) 
        values (%d, from_unixtime(%d), %d, '%s')
        """
            % (lastMsgSeq, lastTime, self.group_id, escape_string(self.uniqueKey))
        )
        self.lastMsgSeq = lastMsgSeq
        self.lastTime = lastTime
    ...
    def judgeSatisfy(self, latestSeq: int, latestTime: int):
        return (
            latestTime - self.lastTime >= self.deltaTime
            and latestSeq - self.lastMsgSeq >= self.deltaMsg
        )
...
class McAdManager(StandardPlugin):
    def __init__(self):
        ...
        for group_id in getPluginEnabledGroups("mcad"):
            self.groups[group_id] = BaseAdvertisementClass(
                self.adword, group_id, self.uniqueKey, self.deltaTime, self.deltaMsg
            )
