...
class HelpFAQ(StandardPlugin):
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        picPath = (
            picPath if os.path.isabs(picPath) else os.path.join(ROOT_PATH, picPath)
        )
...
def get_questions(group_id: int) -> List[str]:
    ...
    mycursor = mydb.cursor()
    mycursor.execute(
        """select `question` from `BOT_FAQ_DATA`.`%d`
...
def get_alldata(group_id: int) -> List[Tuple[str, str, str]]:
    ...
    mycursor = mydb.cursor()
    mycursor.execute(
        """select `question`, `answer`, `group_tag` from `BOT_FAQ_DATA`.`%d`
    where latest = true and deleted = false
    """
        % group_id
    )
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if hasMsg:
            ...
            fuzzy_ans = [
                fza[0]
                for fza in sorted(
                    fuzzy_process.extract(question, questions, limit=5),
                    key=lambda x: x[1],
                    reverse=True,
                )
                if fza[1] > 30
            ...
            if msg.startswith("q"):
                ...
                if len(fuzzy_ans) == 0:
                    ...
                    if hasMsg:
                        ...
                        ans = "[CQ:reply,id=%d]未查询到信息，输入faq ls查看问题列表" % (
                            data["message_id"]
                        )
        send(group_id, ans)
...
class MaintainFAQ(StandardPlugin):
    ...
    def judgeTrigger(self, msg: str, data: Any) -> bool:
        return (
            self.findModPattern.match(msg) != None and data["message_type"] == "group"
        )
    ...
    def faqShow(cmd: str, data):
        ...
        if cmd == "":
            ...
            mycursor.execute(
                """select `group_tag`, `question` from `BOT_FAQ_DATA`.`%d`
            where latest = true and deleted = false
            """
                % groupId
            )
    ...
    def faqAdd(cmd: str, data):
        ...
        if len(qa) == 0:
            ...
            if status:
                send(
                    groupId,
                    "[CQ:reply,id=%d]问题【%s】已经存在，请输入faq edit <key> <new value>进行更改"
                ...
                if status:
                    send(
                        groupId,
                        "[CQ:reply,id=%d]%s\n【%s】"
                        % (data["message_id"], answer, question),
                    )
                else:
    ...
    def faqEdit(cmd: str, data):
        ...
        if len(qa) == 0:
            ...
            if not status:
                send(
                    groupId,
                    '[CQ:reply,id=%d]问题不存在，请先使用"faq add"语句创建该问题' % data["message_id"],
                )
            else:
    ...
    def faqCp(cmd: str, data):
        ...
        if len(qa) == 0:
            ...
            if not status:
                send(
                    groupId,
                    "[CQ:reply,id=%d]【%s】问题不存在，请输入faq ls查看问题列表"
                    % (data["message_id"], prevName),
                )
            else:
                ...
                if status:
                    send(
                        groupId,
                        "[CQ:reply,id=%d]%s\n【%s】"
                        % (data["message_id"], answer, newName),
                    )
                else:
    ...
    def faqDel(cmd: str, data):
        ...
        if len(question) == 0:
            ...
            if not status:
                send(
                    groupId,
                    "[CQ:reply,id=%d]问题不存在，请输入faq ls查看问题列表" % (data["message_id"]),
                )
            else:
                status = update_answer(
                    groupId, question, "", data, delete=True, tag=tag
                )
                if status:
    ...
    def faqAppend(cmd: str, data):
        ...
        if len(qa) == 0:
            ...
            if not status:
                send(
                    groupId,
                    "[CQ:reply,id=%d]【%s】问题不存在" % (data["message_id"], question),
                )
            else:
        ...
        if len(question) == 0:
            ...
            if data["user_id"] not in getGroupAdmins(groupId):
                ...
                if status:
                    ...
                    send(
                        groupId,
                        "[CQ:reply,id=%d]记录【%s】不存在，请输入faq ls查看问题列表"
                        % (data["message_id"], question),
                    )
    ...
    def faqHistory(cmd: str, data):
        ...
        if len(question) == 0:
            ...
            if data["user_id"] not in getGroupAdmins(groupId):
                send(
                    groupId,
                    "[CQ:reply,id=%d]您没有查看记录权限，输入faq help获取问答帮助" % (data["message_id"]),
                )
            else:
                question = question[0]
                ...
                picPath = (
                    picPath
                    if os.path.isabs(picPath)
                    else os.path.join(ROOT_PATH, picPath)
                )
                send(groupId, "[CQ:image,file=files:///%s]" % picPath)
    ...
    def faqExport(cmd: str, data):
        ...
        if data["user_id"] not in getGroupAdmins(groupId):
            ...
            filePath = os.path.join(
                ROOT_PATH, SAVE_TMP_PATH, "faqdata-%d.json" % groupId
            )
            ...
            upload_group_file(
                groupId,
                filePath,
                datetime.datetime.now().strftime("LUBFAQ-%y%m%d.json"),
                "",
            )
    ...
    def faqImport(cmd: str, data):
        ...
        if data["user_id"] not in getGroupAdmins(groupId):
            ...
            targetFiles = [
                f for f in rootFiles if fileNamePattern.match(f["file_name"]) != None
            ]
            if len(targetFiles) == 0:
                send(
                    groupId,
                    "[CQ:reply,id=%d]群文件根目录下没有找到任何名为LUBFAQ**.json的文件"
                    % (data["message_id"]),
                )
                return
            ...
            if targetFile["file_size"] > 10 * 1024 * 1024:
                send(
                    groupId,
                    "[CQ:reply,id=%d]目标文件 %s 超过10M，暂不支持"
                    % (data["message_id"], targetFile["file_name"]),
                )
                return
            fileUrl = get_group_file_url(
                targetFile["group_id"], targetFile["file_id"], targetFile["busid"]
            )
            if fileUrl == None:
            ...
            try:
                ...
                for faq in faqData:
                    if (
                        "question" not in faq.keys()
                        or "answer" not in faq.keys()
                        or "tag" not in faq.keys()
                    ):
                        send(
                            groupId, "[CQ:reply,id=%d]faq内容解析失败" % (data["message_id"])
                        )
                        return
                ...
                if len(conflictKeys) != 0:
                    send(
                        groupId,
                        "[CQ:reply,id=%d]检测到key冲突，冲突key为：\n%s\n\n请手动添加这些key"
                        % (data["message_id"], "、".join(conflictKeys)),
                    )
                failedKeys: List[str] = []
                ...
                if len(failedKeys) != 0:
                    send(
                        groupId,
                        "[CQ:reply,id=%d]识别到faq文件%s，由用户 %s(%d) 上传于%s。由于数据库错误，下面的问题添加失败：\n%s\n\n，其余问题添加成功（成功添加%d个问题）"
                        % (
                            data["message_id"],
                            targetFile["file_name"],
                            targetFile["uploader_name"],
                            targetFile["uploader"],
                            time.strftime(
                                "%Y-%m-%d %H:%M:%S",
                                time.localtime(targetFile["upload_time"]),
                            ),
                            "、".join(failedKeys),
                    ...
                    send(
                        groupId,
                        "[CQ:reply,id=%d]识别到faq文件%s，由用户 %s(%d) 上传于%s。成功添加%d个问题。"
                        % (
                            data["message_id"],
                            targetFile["file_name"],
                            targetFile["uploader_name"],
...
def drawQuestionCardByPinyin(questions: List[str], group_id: int) -> str:
    ...
    helpCards = ResponseImage(
        ...
        layout="two-column",
        width=1280,
        cardBodyFont=ImageFont.truetype(
...
def draw_answer_history(group_id: int, question: str) -> str:
    ...
    if len(results) == 0:
        helpCards.addCard(ResponseImage.NoticeCard(title="暂无历史记录"))
    savePath = os.path.join(
        ROOT_PATH, SAVE_TMP_PATH, "%d-faq-history.png" % (group_id,)
    )
    helpCards.generateImage(savePath)
    return savePath
