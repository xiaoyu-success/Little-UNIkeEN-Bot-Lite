    ...
    def faqImport(cmd: str, data):
        ...
        if data['user_id'] not in getGroupAdmins(groupId):
            ...
            if len(targetFiles) == 0:
                send(groupId,
                     '[CQ:reply,id=%d]群文件根目录下没有找到任何名为LUBFAQ**.json的文件' % (data['message_id']))
                return
            targetFile = max(targetFiles, key=lambda f: f['upload_time'])
            if targetFile['file_size'] > 10 * 1024 * 1024:
                send(groupId,
                     '[CQ:reply,id=%d]目标文件 %s 超过10M，暂不支持' % (data['message_id'], targetFile['file_name']))
                return
                ...
                if len(failedKeys) != 0:
                    send(groupId,
                         '[CQ:reply,id=%d]识别到faq文件%s，由用户 %s(%d) 上传于%s。由于数据库错误，下面的问题添加失败：\n%s\n\n，其余问题添加成功（成功添加%d个问题）' % (
                             data['message_id'], targetFile['file_name'], targetFile['uploader_name'],
                             targetFile['uploader'],
