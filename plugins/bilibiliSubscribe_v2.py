    ...
    try:
        ...
        if dynamicType == 8:
            ...
            body = "".join(
                [
                    "标题： ",
                    dynamicInfo["card"]["title"],
                    "\n",
                    "简介： ",
                    dynamicInfo["card"]["desc"],
                    "\n",
                    "IP： ",
                    dynamicInfo["card"]["pub_location"],
                    "\n",
                    "时长： ",
                    durationToStr(dynamicInfo["card"]["duration"]),
                    "\n",
                    "链接： ",
                    dynamicInfo["card"]["short_link_v2"],
                    "\n",
                    "分辨率： ",
                    dimensionToStr(dynamicInfo["card"]["dimension"]),
                ]
            )
            ...
            card.addCard(
                ResponseImage.RichContentCard(
                    [
                        ...
                        (
                            "subtitle",
                            pubtime.strftime("更新于 %Y-%m-%d %H:%M:%S"),
                        ),
                        (
                            "keyword",
                            "类型： 视频",
            ...
            if "origin" in dynamicInfo["card"].keys():
                ...
                if "publish_time" in origin.keys():
                    originPubtime = datetime.datetime.fromtimestamp(
                        origin["publish_time"]
                    )
                ...
                card.addCard(
                    ResponseImage.RichContentCard(
                        [
                            ...
                            (
                                "subtitle",
                                originPubtime.strftime("更新于 %Y-%m-%d %H:%M:%S")
                                if originPubtime != None
                                else "未知的发布时间",
                            ),
                            (
                                "keyword",
            ...
            for addonCard in dynamicInfo.get("display", {}).get("add_on_card_info", []):
                if "ugc_attach_card" not in addonCard.keys():
                    continue
...
class BilibiliSubscribe(StandardPlugin):
    ...
    def judgeTrigger(self, msg: str, data: Any) -> bool:
        return (
            self.pattern1.match(msg) != None
            or self.pattern2.match(msg) != None
            or self.pattern3.match(msg) != None
        )
    ...
    def onStateChange(self, nextState: bool, data: Any) -> None:
        group_id = data["group_id"]
        if nextState or group_id not in self.groupUps.keys():
            return
...
class BilibiliMonitor(CronStandardPlugin):
    ...
    def tick(self) -> None:
        ...
        try:
            ...
            if prevMeta != None and prevMeta[1] == dynamicId:
                return
            # 校验upload time
            if uploadTime > int(datetime.datetime.now().timestamp()):
                warning(
                    "invalid uploadTime {} in bilibili dynamic {}".format(
                        uploadTime, dynamicId
                    )
                )
                return
            ...
            imgPath = os.path.join(
                ROOT_PATH, SAVE_TMP_PATH, "biliDynamic-%d.png" % dynamicId
            )
            succ, drawInfo = drawDynamicCard(latestDynamic, imgPath)
            if not succ:
                warning(
                    "draw bilibili dynamic cards failed! uid = %d, bvid = %d, reason: %s"
