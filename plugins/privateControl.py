...
class HelpInGroup(StandardPlugin):
    ...
    def judgeTrigger(self, msg: str, data: Any) -> bool:
        return (
            self.triggerPattern.match(msg) != None and data["user_id"] in ROOT_ADMIN_ID
        )
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if grpId not in APPLY_GROUP_ID:
            send(
                target,
                "[CQ:reply,id=%d]该群尚未加入白名单" % data["message_id"],
                data["message_type"],
            )
        elif self.plugins == None:
            send(
                target,
                "[CQ:reply,id=%d]BUG: self.plugins==None, 请上报管理员" % data["message_id"],
                data["message_type"],
            )
        else:
...
class GroupApply(StandardPlugin):
    ...
    def judgeTrigger(self, msg: str, data: Any) -> bool:
        return (
            startswith_in(msg, ["-enable", "-disable"])
            and data["user_id"] in ROOT_ADMIN_ID
        )
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if self.onPattern.match(msg) != None:
            ...
            if len(description) > 100:
                send(
                    target,
                    "[CQ:reply,id=%d]添加失败，群描述长度超限" % data["message_id"],
                    data["message_type"],
                )
            else:
            ...
            if groupId in APPLY_GROUP_ID:
                ...
                send(
                    target,
                    "[CQ:reply,id=%d]该群不在白名单中" % data["message_id"],
                    data["message_type"],
                )
        else:
            send(
                target,
                "[CQ:reply,id=%d]指令识别失败，请输入-help获取帮助" % data["message_id"],
                data["message_type"],
            )
        return "OK"
...
class PrivateControl(StandardPlugin):
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        if prevConf == None:
            send(
                target,
                f'[CQ:reply,id={data["message_id"]}]不存在群 {groupId} 或此群【{pluginName}】插件尚未初始化',
                data["message_type"],
            )
