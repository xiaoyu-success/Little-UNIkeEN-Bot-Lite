...
class ShowHelp(StandardPlugin):
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        def getTargetPluginList(
            pluginList: List[StandardPlugin], helpWords: str
        ) -> List[StandardPlugin]:
            if helpWords == "":
                return pluginList
            if self.subdevidePattern.match(helpWords) != None:
                currentPlugin, nextHelpWords = self.subdevidePattern.findall(helpWords)[
                    0
                ]
                for p in pluginList:
                    if not isinstance(p, PluginGroupManager):
                        continue
...
def drawHelpCard(pluginList, group_id) -> str:
    helpCards = ResponseImage(
        ...
        cardSubtitleFont=ImageFont.truetype(
            os.path.join(FONTS_PATH, "SourceHanSansCN-Medium.otf"), 24
        ),
    )
                        ...
                        cardPluginList.append(
                            (
                                ...
                                (
                                    infoDict["description"]
                                    + ":  "
                                    + infoDict["commandDescription"]
                                ),
                            )
                        )
                except KeyError:
                    warning(
                        "meet KeyError when getting help, plugin: {}".format(plugin)
                    )
        else:
            item: StandardPlugin
            flag = True
...
class ServerMonitor(StandardPlugin):
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        ...
        save_path = (
            ...
            else os.path.join(ROOT_PATH, save_path)
        )
