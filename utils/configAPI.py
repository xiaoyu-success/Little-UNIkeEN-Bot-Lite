...
def readGlobalConfig(
    groupId: Union[None, int], pluginName: str
) -> Union[dict, Any, None]:
    """读global config
    @groupId
    ...
    if groupId == None:
        ...
        try:
            mycursor.execute(
                "SELECT groupId, json_extract(groupConfig,'$.%s') from globalConfig"
                % pluginName
            )
...
def writeGlobalConfig(groupId: Union[None, int], pluginName: str, value: Any):
    ...
    if groupId == None:
        try:
            mycursor.execute(
                "update globalConfig set groupConfig=json_set(groupConfig, '$.%s', cast('%s' as json))"
        ...
        try:
            mycursor.execute(
                "insert ignore into globalConfig(groupId, groupConfig, groupAdmins) values (%d, '{}', '[]')"
                % groupId
            )
            mycursor.execute(
        ...
        warning(
            "unknow groupId type in writeGlobalConfig: groupId = {}".format(groupId)
        )
...
def getPluginEnabledGroups(pluginName: str) -> List[int]:
    ...
    try:
        mycursor.execute(
            "select groupId from globalConfig \
            where json_extract(groupConfig, '$.%s.enable') = true"
            % escape_string(pluginName)
        )
...
def getGroupAdmins(groupId: int) -> List[int]:
    ...
    try:
        mycursor.execute(
            "select groupAdmins from globalConfig where groupId = %s" % (groupId)
        )
        ...
        if len(result) <= 0:
            ...
            if not isinstance(result, list) or any(
                [not isinstance(x, int) for x in result]
            ):
...
def addGroupAdmin(groupId: int, adminId: int):
    ...
    if not isinstance(groupId, int) or not isinstance(adminId, int):
        warning(
            "error groupId type or adminId type in addGroupAdmin: groupId = {}, adminId = {}".format(
                groupId, adminId
            )
        )
        return
...
def setGroupAdmin(groupId: int, adminIds: List[int]):
    ...
    """
    if not isinstance(adminIds, list) or any(
        [not isinstance(x, int) for x in adminIds]
    ):
        warning("error admin type, groupId = %d" % groupId)
        return
    mydb, mycursor = newSqlSession()
    try:
        ...
        mycursor.execute(
            "update globalConfig set groupAdmins='%s' where groupId=%d;"
            % (json.dumps(adminIds), groupId)
        )
    except mysql.connector.Error as e:
...
def delGroupAdmin(groupId: int, adminId: int):
    ...
    if not isinstance(groupId, int) or not isinstance(adminId, int):
        warning(
            "error groupId type or adminId type in delGroupAdmin: groupId = {}, adminId = {}".format(
                groupId, adminId
            )
        )
        return
