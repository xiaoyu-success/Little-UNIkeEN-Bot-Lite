...
def send(id: int, message: str, type: str = "group") -> None:
    ...
    if type == "group":
        params = {"message_type": type, "group_id": id, "message": message}
        ...
        params = {"message_type": type, "user_id": id, "message": message}
        requests.get(url, params=params)
...
def get_group_msg_history(group_id: int, message_seq: Union[int, None] = None) -> list:
    ...
    try:
        ...
        if messageHistory["status"] != "ok":
            if (
                messageHistory["msg"] == "MESSAGES_API_ERROR"
                or messageHistory["msg"] == "GROUP_INFO_API_ERROR"
            ):
                print(
                    "group {} meet '{}' error".format(group_id, messageHistory["msg"])
                )
            else:
                warning(
