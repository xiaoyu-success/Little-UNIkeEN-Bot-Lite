...
# 字体预定义常量
FONT_SYHT_M42 = ImageFont.truetype(
    os.path.join(FONTS_PATH, "SourceHanSansCN-Medium.otf"), 42
)
FONT_SYHT_M32 = ImageFont.truetype(
    os.path.join(FONTS_PATH, "SourceHanSansCN-Medium.otf"), 32
)
FONT_SYHT_M28 = ImageFont.truetype(
    os.path.join(FONTS_PATH, "SourceHanSansCN-Medium.otf"), 28
)
FONT_SYHT_M24 = ImageFont.truetype(
    os.path.join(FONTS_PATH, "SourceHanSansCN-Medium.otf"), 24
)
FONT_SYHT_M18 = ImageFont.truetype(
...
class ResponseImage:
    ...
    class BlankCard(Card):
        ...
        def __init__(self, size: int = 0, backColor: tuple = None):
            params = vars()
    ...
    def drawRoundedRectangle(
        ...
        r=8,
        border=False,
        borderColor=PALETTE_GREY_BORDER,
        borderWidth=1,
        target=None,
    ):
        if target == None:
    ...
    def calcHeight(self):
        ...
        height += (
            2 * txt_size_18[1] + SPACE_NORMAL + SPACE_ROW
            if self.footer != ""
            else txt_size_18[1] + SPACE_NORMAL
        ) + 15  # 加页脚距离
        ...
        for i in range(len(cardList)):
            ...
            if cardList[i]["style"] in ["blank"]:
                if (
                    cardList[i].get("size") != None
                    and cardList[i]["size"] >= -SPACE_NORMAL
                ):
                ...
                if cardList[i]["style"] == "normal":
                    ...
                    if cardList[i].get("illustration") != None:
                        ...
                        if image.width > w_card - 2 * SPACE_NORMAL:
                            h_card += int(
                                image.height * (w_card - 2 * SPACE_NORMAL) / image.width
                            )
                            h_card += SPACE_ROW
                        else:
                            h_card += image.height
                ...
                titleColor = (
                    PALETTE_BLACK
                    if cardList[i].get("titleFontColor") == None
                    else cardList[i]["titleFontColor"]
                )
                subtitleColor = (
                    PALETTE_GREY_SUBTITLE
                    if cardList[i].get("subtitleFontColor") == None
                    else cardList[i]["subtitleFontColor"]
                )
                bodyColor = (
                    PALETTE_GREY_SUBTITLE
                    if cardList[i].get("bodyFontColor") == None
                    else cardList[i]["bodyFontColor"]
                )
                if cardList[i].get("title") != None:
                    parsed, h = self.parseLine(
                        cardList[i]["title"], self.cardTitleFont, txtWidthLimit
                    )
                    cardList[i]["content"].extend(
                        [("title", t, titleColor) for t in parsed]
                    )
                    h_card += h
                if cardList[i].get("subtitle") != None:
                    parsed, h = self.parseLine(
                        cardList[i]["subtitle"], self.cardSubtitleFont, txtWidthLimit
                    )
                    cardList[i]["content"].extend(
                        [("subtitle", t, subtitleColor) for t in parsed]
                    )
                    h_card += h
                if cardList[i].get("keyword") != None:
                    parsed, h = self.parseLine(
                        cardList[i]["keyword"], self.cardSubtitleFont, txtWidthLimit
                    )
                    cardList[i]["content"].extend(
                        [("keyword", t, self.primaryColor) for t in parsed]
                    )
                    h_card += h
                if cardList[i].get("body") != None:
                    parsed, h = self.parseLine(
                        cardList[i]["body"], self.cardBodyFont, txtWidthLimit
                    )
                    cardList[i]["content"].extend(
                        [("body", t, bodyColor) for t in parsed]
                    )
                    h_card += h
                if (cardList[i].get("icon") != None) and h_card <= (
                    2 * SPACE_NORMAL + 90
                ):
                    ...
                    for cnt in cardList[i]["raw_content"]:
                        ...
                        if cnt[0] == "illustration":
                            ...
                            if image.width > txtWidthLimit:
                                h_card += int(
                                    image.height * (txtWidthLimit) / image.width
                                )
                                h_card += SPACE_ROW
                            else:
                                h_card += image.height
    ...
    def drawImage(self):
        ...
        self.drawRoundedRectangle(
            x1=width / 2 - txt_size[0] / 2 - 15,
            y1=40,
            x2=width / 2 + txt_size[0] / 2 + 15,
            y2=txt_size[1] + 70,
            fill=self.titleColor,
        )
        draw.text(
            (width / 2 - txt_size[0] / 2, 55),
            self.title,
            fill=PALETTE_WHITE,
            font=FONT_HYWH_36,
        )
        ...
        draw.text(
            (width / 2 - txt_size[0] / 2, height - SPACE_NORMAL - txt_size[1]),
            "Powered By Little-UNIkeEN-Bot",
            fill=PALETTE_GREY_CONTENT,
            font=FONT_SYHT_M18,
        )
        if self.footer != "":
            ...
            draw.text(
                (
                    width / 2 - txt_size[0] / 2,
                    height - SPACE_NORMAL - SPACE_ROW - 2 * txt_size[1],
                ),
                self.footer,
                fill=PALETTE_GREY_CONTENT,
                font=FONT_SYHT_M18,
        ...
        for card in self.cardList:
            if self.layout == "normal":
                ...
                cardLeft = (
                    SPACE_DOUBLE
                    if i <= self.columnSep
                    else (SPACE_DOUBLE + SPACE_NORMAL + card["width"])
                )
                cardRight = (
                    SPACE_DOUBLE + card["width"]
                    if i <= self.columnSep
                    else (width - SPACE_DOUBLE)
                )
            ...
            y_top = top
            backColor = (
                PALETTE_WHITE if card.get("backColor") == None else card["backColor"]
            )
            if card["style"] == "blank":
                self.drawRoundedRectangle(
                    x1=cardLeft,
                    y1=top,
                    x2=cardRight,
                    y2=top + card["height"] - SPACE_NORMAL,
                    fill=backColor,
                    border=True,
                )
                top += card["height"]
                self.blankList.append(
                    (cardLeft, top, cardRight, top + card["height"] - SPACE_NORMAL)
                )
                continue
            self.drawRoundedRectangle(
                x1=cardLeft,
            ...
            titleColor = (
                PALETTE_BLACK
                if card.get("titleFontColor") == None
                else card["titleFontColor"]
            )
            subtitleColor = (
                PALETTE_GREY_SUBTITLE
                if card.get("subtitleFontColor") == None
                else card["subtitleFontColor"]
            )
            bodyColor = (
                PALETTE_GREY_SUBTITLE
                if card.get("bodyFontColor") == None
                else card["bodyFontColor"]
            )
            if card.get("content") != None:
                for line in card["content"]:
                    if line[0] == "separator":
                        ...
                        y_top += SPACE_ROW
                        draw.line(
                            (x_l, y_top, cardRight - SPACE_NORMAL, y_top),
                            PALETTE_GREY_BORDER,
                            1,
                        )
                        y_top += 2 * SPACE_ROW + 1
                    ...
                    if line[0] in ["progressBar"]:
                        ...
                        self.drawRoundedRectangle(
                            x_l, y_top, x_r, y_top + SPACE_ROW, fill=clrback, r=3.5
                        )
                        if line[1] > 0:
                            self.drawRoundedRectangle(
                                x_l,
                                y_top,
                                x_l
                                + (x_r - x_l) * (line[1] if line[1] >= 0.01 else 0.01),
                                y_top + SPACE_ROW,
                                fill=clrfront,
                                r=3.5,
                            )
                    ...
                    if line[0] in ["illustration"]:
                        ...
                        if illu.width > card["width"] - 2 * (SPACE_NORMAL):
                            illu = illu.resize(
                                (
                                    int(card["width"] - 2 * SPACE_NORMAL),
                                    int(
                                        illu.height
                                        * (card["width"] - 2 * SPACE_NORMAL)
                                        / illu.width
                                    ),
                                )
                            )
                        self.img.paste(
                            illu,
                            (int(cardLeft + (card["width"] - illu.width) / 2), y_top),
                        )
                    ...
                    x_l = (
                        cardLeft + (card["width"] - txt_size[0]) / 2
                        if card["style"] == "notice"
                        else x_left
                    )
            ...
            if card["style"] == "notice" and card.get("illustration") != None:
                ...
                self.img.paste(
                    illu, (int(cardLeft + (card["width"] - illu.width) / 2), y_top)
                )
                y_top += illu.height + SPACE_ROW
            top += card["height"]
