...
class ScheduleStandardPlugin(BaseTimeSchedulePlugin):
    ...
    def schedule(self, hour: Union[str, int] = 0, minute: Union[str, int] = 0) -> Job:
        ...
        @return: this job
        """
        return BaseTimeSchedulePlugin.scheduler.add_job(
            self._tick, "cron", hour=hour, minute=minute
        )
...
class CronStandardPlugin(BaseTimeSchedulePlugin):
    ...
    def start(self, startTime: float, intervalTime: float) -> Job:
        ...
        """
        return BaseTimeSchedulePlugin.scheduler.add_job(
            self._tick, "interval", seconds=intervalTime
        )
...
class PluginGroupManager(StandardPlugin):
    ...
    def __init__(self, plugins: List[StandardPlugin], groupName: str) -> None:
        ...
        PluginGroupManager.refreshPluginStatusHandles.append(
            (groupName, self._refreshPluginStatus)
        )
    ...
    def _checkGroupInfo(self):
        ...
        if not all(["group" in p.getPluginInfo()["usePlace"] for p in self.plugins]):
            warning(
                "all plugins should be able to use in group, error in {}".format(
                    self.groupName
                )
            )
    ...
    def executeEvent(self, msg: str, data: Any) -> Union[None, str]:
        if self.readyPlugin == None:
            warning(
                "logic error in PluginGroupManager: executeEvent self.readyPlugin == None"
            )
            return None
        if self.readyPlugin in ["enable", "disable"]:
            ...
            try:
                ...
                warning(
                    "logic error in PluginGroupManager [{}]: {}".format(
                        self.groupName, e
                    )
                )
                return None
    ...
    def queryEnabled(self, groupId: int) -> bool:
        if groupId not in self.enabledDict.keys():
            writeGlobalConfig(
                groupId,
                self.groupName,
                {"name": self.groupName, "enable": self.defaultEnabled},
            )
