...
def get_guild_channel_list(guild_id: str, no_cache: bool) -> List[Dict[str, Any]]:
    ...
    params = {"guild_id": guild_id, "no_cache": no_cache}
    try:
...
def send_guild_channel_msg(
    guild_id: str, channel_id: str, message: str
    ...
    params = {"guild_id": guild_id, "channel_id": channel_id, "message": message}
    print(params)
    try:
        req = requests.get(url=url, params=params)
        req = req.json()
...
def get_guild_member_list(
    guild_id: str, next_token: Optional[str] = None
) -> Dict[str, Any]:
    """获取频道成员列表
    @guild_id: 频道ID
